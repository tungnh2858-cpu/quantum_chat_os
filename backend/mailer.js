...
function getTransporter() {
  ...
  transporter = nodemailer.createTransport({
    ...
    // Fail fast instead of hanging the request (e.g. login/OTP) if SMTP is slow or unreachable.
...
async function sendMail(to, subject, text) {
  ...
  if (!t) {
    console.log(`\n[Quantum Chat OS Mailer - SMTP CHƯA CẤU HÌNH, in ra console thay vì gửi thật]\nĐến: ${to}\nTiêu đề: ${subject}\nNội dung:\n${text}\n`);
  ...
  try {
    ...
    console.error('[Quantum Chat OS Mailer] Gửi email thất bại, in ra console thay thế:', e.message);
    console.log(`Đến: ${to}\nTiêu đề: ${subject}\nNội dung:\n${text}`);
    return { sent: false, devFallback: true };
  }
}
