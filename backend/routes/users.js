...
// POST /api/users  (admin creates account: student / teacher / developer / admin)
router.post('/', requireAuth, requireAdmin, (req, res) => {
  const { username, password, role, fullName, email, allowedTools, requireLoginOtp, emailOnLogin } = req.body || {};
  if (!username || !password || !role) return res.status(400).json({ error: 'Thiếu thông tin bắt buộc.' });
  if (!['admin', 'teacher', 'student', 'developer'].includes(role)) return res.status(400).json({ error: 'Vai trò không hợp lệ.' });
  ...
  const newUser = {
    ...
    security: {
      ...
      allowedTools: Array.isArray(allowedTools) && allowedTools.length ? allowedTools : null
    },
    status: 'active',
...
// PUT /api/users/:id/permissions  (admin sets allowed tools + login-security flags)
router.put('/:id/permissions', requireAuth, requireAdmin, (req, res) => {
  const { allowedTools, requireLoginOtp, emailOnLogin } = req.body || {};
  const db = getDB();
  const user = db.users.find(u => u.id === req.params.id);
  if (!user) return res.status(404).json({ error: 'Không tìm thấy tài khoản.' });
  if (!user.security) user.security = { requireLoginOtp: false, emailOnLogin: false, allowedTools: null };
  if (allowedTools !== undefined) user.security.allowedTools = Array.isArray(allowedTools) && allowedTools.length ? allowedTools : null;
  if (requireLoginOtp !== undefined) user.security.requireLoginOtp = !!requireLoginOtp;
  if (emailOnLogin !== undefined) user.security.emailOnLogin = !!emailOnLogin;
  saveDB(db);
  res.json({ user: sanitize(user) });
});
