...
router.post('/login', async (req, res) => {
  ...
  if (requiresOtp) {
    ...
    const mailResult = await sendMail(
      user.email,
      'EduPulse - Mã xác nhận đăng nhập',
  ...
  if (user.security && user.security.emailOnLogin && user.email) {
    sendMail(user.email, 'EduPulse - Thông báo đăng nhập', `Tài khoản ${user.username} vừa đăng nhập lúc ${new Date().toLocaleString('vi-VN')}. Nếu không phải bạn, hãy đổi mật khẩu ngay.`)
