...
const EduLayout = (() => {
  ...
  function render(activeId) {
    ...
    const items = NAV.filter(n => {
      ...
      // Per-account tool restriction set by Admin (dashboard/settings always visible).
      if (user.role !== 'admin' && allowedTools && !['dashboard', 'settings'].includes(n.id) && !allowedTools.includes(n.id)) return false;
      return true;
    }).map(n => `
        ...
        <div>
          <div class="p-6 border-b border-white/5 flex items-center justify-between">
            <div class="flex items-center gap-3">
              <i class="fa-solid fa-atom text-cyan-400 text-3xl drop-shadow-[0_0_8px_rgba(0,240,255,0.8)]"></i>
